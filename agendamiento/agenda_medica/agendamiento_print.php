...
$pdf->SetX(80);
$pdf->Cell(2, 10, 'Dirección: Colocar Direccion', 0, 1, 'L');
$pdf->SetX(80);
$pdf->Cell(2, 10, 'Teléfono: 0981544989', 0, 1, 'L');
$pdf->SetX(80);
$pdf->Cell(2, 10, 'Correo: amoryvida@gmail.com', 0, 1, 'L');
...
if (!empty($cabeceras)) {
    foreach ($cabeceras as $cabecera) {
        ...
        $pdf->SetFont('', '', 11);
        $pdf->Cell(130, 2, "Doctor: (" . $cabecera['cod_doctor'] . ") " . strtoupper($cabecera['doctor']), 0, '', 'L');
        $pdf->Cell(80, 2, "Fecha: " . $cabecera['agen_fecha'], 0, 1);
        $pdf->Cell(130, 2, "Especialidad: " . strtoupper($cabecera['descrip_espec']), 0, '', 'L');
        $pdf->Cell(80, 2, "Día: " . $cabecera['dia_descri'], 0, 1);
        $pdf->Cell(130, 2, "Turno: " . $cabecera['descrip_tur'], 0, '', 'L');
        $pdf->Cell(80, 2, "Sala: " . $cabecera['descrip_sala'], 0, 1);
        $pdf->Ln();
    }
} else {
    // If no records found
